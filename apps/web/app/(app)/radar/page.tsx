...
import { Clipboard, ExternalLink, Link2, MessageSquareText, PlusCircle, Search, TrendingUp } from "lucide-react";
...
export default function RadarPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Falha ao buscar oportunidades.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function generateAffiliateLink(offerId: string) {
    ...
    try {
      ...
      if (updated.affiliateUrl) {
        ...
        setMessage(copied ? "Link afiliado gerado e copiado." : "Link afiliado gerado.");
      } else {
        setError(result.result.message || "Geracao automatica nao retornou link. Cole o link afiliado final no card.");
      }
    } catch (err) {
  ...
  async function saveAffiliateLink(offerId: string) {
    ...
    try {
      ...
      setMessage("Link afiliado salvo.");
    } catch (err) {
  ...
  async function copyAffiliateLink(offer: Offer) {
    ...
    setError("");
    setMessage(copied ? "Link afiliado copiado." : "Nao foi possivel copiar automaticamente.");
  }
  ...
  function moveToGenerated(offerId: string, updated: OfferUpdate) {
    ...
    setGroups((current) =>
      current.map((group) => ({
        ...group,
        offers: group.offers.filter((offer) => offer.id !== offerId),
        count: group.offers.filter((offer) => offer.id !== offerId).length
      }))
    );
  }
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {offer.affiliateUrl ? <p className="mt-1 line-clamp-1 text-xs text-leaf">Link afiliado pronto</p> : null}
            </div>
          </div>
    ...
    <>
      ...
      {groups.length ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {groups.map((group) => (
            <span key={group.category} className="rounded-md bg-mist px-3 py-1 text-xs font-semibold text-leaf">
      ...
      <section className="mt-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <TrendingUp size={18} className="text-leaf" aria-hidden />
            <h2 className="text-base font-semibold text-ink">Oportunidades encontradas</h2>
          </div>
      ...
      <section className="mt-6">
        ...
        {generatedOffers.length ? (
          ...
          <Panel className="text-sm text-[var(--muted)]">Os links gerados pelo Radar vao aparecer aqui para campanha.</Panel>
        )}
      </section>
    </>
  );
